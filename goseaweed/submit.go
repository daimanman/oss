...
//接受Multipart 文件
func NewReaderPart(reader io.Reader, fileHeader *multipart.FileHeader) (ret FilePart, err error) {
	ret.Reader = reader
...
//根据文件id获取下载url
func (sw *Seaweed) GetFileDownloadUrl(fid string, params map[string]string) string {

	volumeServer, err := sw.LookupFileIdWithParams(fid, "", false, params)
	if err != nil {
		log.Printf("get download err %v \n", err)
		return ""
	}
	return volumeServer
}
